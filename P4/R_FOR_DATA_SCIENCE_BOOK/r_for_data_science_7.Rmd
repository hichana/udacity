...
Appears fair diamonds (lowest quality) may have the highest average price - maybw because frequency polygons are hard to interpret. Try a boxplot:
...
ggplot(data = faithful) +
  ...
  xlab("eruption length") + ylab("time waiting between eruptions")
```
...
Once strong relationship between carat and price removed, can use box plot to see relationship between cut and price - so relative to their size, better quality diamonds are more expensive.

Revisit book to see what this is:
```{r}
ggplot(data = diamonds2) + 
